...
#Specify the format type of each column in the input file:
...
#Remove mice with incorrect data!
data.in = subset(data.in, !(data.in$MID %in% c("217-01", "703-029")))

#Add Cisplatin identifier column
data.in$Cisplatin = as.numeric(grepl("Cisplatin", data.in$Drug))

#Subset the data to only contain control and Cisplatin data
data.sub.1 = subset(data.in, (data.in$Control == 1) | (data.in$Cisplatin == 1))

#Determine which studies have Cisplatin
#-Studies without Cisplatin were confirmed in Excel file.
#-All variants of Cisplatin name (e.g., Cis+, Cis/, Cis, mispellings) were corrected to "Cisplatin" in Excel file!
agg.cis = as.data.frame(aggregate(data.sub.1$Cisplatin, by = list(data.sub.1$Study), FUN=max))

#Subset out any studies that do not have Cisplatin
agg.cis.0 = subset(agg.cis, agg.cis$x == 0)

#data.sub.2 contains all data on studies that have Cisplatin and control
data.sub.2 = subset(data.sub.1, !(data.sub.1$Study %in% as.character(agg.cis.0$Group.1)))

#Exclude the following dataset that did not have Cisplatin only treatment
data.sub.2 = subset(data.sub.2, ((data.sub.2$Study != "TM00231")))

#Determine if any study has more than 1 Cisplatin group (be sure that each group is compared to control); this is rare but ok
agg.group = aggregate(data.sub.2$Drug, by = list(data.sub.2$Study), FUN=unique)

#7 studies have more than 1 Cisplatin group

#Remove any groups that are combination therapy.
patterns = c("Cisplatin+", "Cisplatin +", "Cisplatin/")

#Remove combination therapies with Group name including "Cisplatin+"
data.sub.2.a = data.sub.2[grep("Cisplatin\\+", data.sub.2$Drug, invert=T),]

#Remove combination therapies with Group name including "Cisplatin +"
data.sub.2.b = data.sub.2.a[grep("Cisplatin \\+", data.sub.2.a$Drug, invert=T),]

#Remove combination therapies with Group name including "Cisplatin/"
data.sub.2.c = data.sub.2.b[grep("Cisplatin\\/", data.sub.2.b$Drug, invert=T),]
...
#For each of the studies with both Cisplatin and Control groups perform the following:
i = 1
...
for(i in c(1:70)){
  ...
  #Use resampling to calcuate TC, rate-based TC, and mRECIST for each Cisplatin (treated group) and Control groups for each day
  j = 1
  
  #Determine the groups in the study
  group.uni = unique(study.sub.3$Drug)
  cis.uni = group.uni[grepl("Cisplatin", group.uni)]
  con.uni = group.uni[!grepl("Cisplatin", group.uni)]
  
  #For each Cisplatin group perform the resampling data analyses
  for(j in 1:length(cis.uni)){
    cis.grp = cis.uni[j]
    con.grp = con.uni
    
    #Dataset containing only the Cisplatin group
    ...
    for(k in 1:length(max.all.o)){
      #Subset the Cisplatin and Control data for only the day specificed (k)
      day.cis.1 = subset(data.cis, data.cis$Day_new == max.all.o[k])
      day.con.1 = subset(data.con, data.con$Day_new == max.all.o[k])
      
      #Subset the Cisplatin and Control data for the first day (0) to the last day specified (k)
      ...
      #Determine the number of replicates in the Cisplatin and Control groups  
      day.cis.1.n = length(unique(day.cis.rate$MID))
      day.con.1.n = length(unique(day.con.rate$MID))
      
      #If Cisplatin and Control groups do not have the same replicate size, 
      #then remove the most deviant sample(s) (according to day 0) until replicate size is equal.
      if(day.cis.1.n > day.con.1.n){
        #Determine the difference in replicate size between Cisplatin and Control groups
      ...
      for(m in 1:length(rep.all)){
        ...
        #Record the full population TC value, Cisplatin and Control volume mean and standard deviation.
        if(m == 1){
        ...
        names(data.out) = c("Study", "cis_Group", "Day", "N",
                            "cis_omit_N", "cis_omit_Samples", "con_omit_N", "con_omit_Samples",
                            "cis_day_Size1K", "con_day_Size1K", 
                            "TC_full","TC_diff_05", "TC_diff_10", "TC_cat", "tc.pow.l.10", "tc.pow.g.10", "tc.pow.l.42", "tc.pow.g.42",
                            "Vol_cis_diff", "Vol_con_diff",
                            "RTC_full", "RTC_diff_05", "RTC_diff_10", "RTC_cat", "rtc.pow.l.10", "rtc.pow.g.10", "rtc.pow.l.42", "rtc.pow.g.42",
                            "Slope_cis_diff", "Slope_con_diff",
                            "RTV_cis_diff", "RTV_con_diff", 
                            ...
                            "CisV_full","CisV_diff_05", "CisV_diff_10", "CisV_cat",
...
write.table(data.out.df, "Malcolm_resampling_cisplatin_vs_control_results_v16.txt", sep="\t", row=F, quote=F)
write.table(data.in.df.all, "Malcolm_resampling_cisplatin_vs_control_input_v16.txt", sep="\t", row=F, quote=F)
