    ...
    for i in range(1, k_max + 1):
        ...
        for s in range(0, s_max):
            for m in range(0, m_max):
                results_dir = data_dir + '/' + str(k) + '/' + str(s) + '/' + str(m)
                if eval_type == 'train':
...
def plot_diagonal_and_save_max(write_fn, max_fn, read_train, read_test, n_seeds, n_restarts):
    ...
    # get bestK via spline fitting
    x = range(0, len(test_vals))
    spl = UnivariateSpline(x, test_vals)
    newy = spl(x)
    spl_argmaxK = np.argmax(newy) + 1 # because k is 1-indexed
    helpers.write_pickle(spl_argmaxK, max_fn)
    #======
    # create plot
